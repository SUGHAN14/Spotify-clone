import React, { useContext } from 'react';
import { assets, songsData } from '../assets/frontend-assets/assets';
import { PlayerContext } from "../context/PlayerContext";

const Player = () => {
  const { seekBar, seekBg, playStatus, play, pause, track, time, previous, next, seekSong } =
    useContext(PlayerContext);

  return (
    <div className="fixed bottom-0 w-full h-[10%] bg-black flex justify-between items-center text-white px-4">
      <div className="flex items-center gap-4">
        <img className="w-12" src={songsData[0].image} alt="" />
        <div>
          <p className="text-sm">{songsData[0].name}</p>
          <p className="text-xs">{songsData[0].desc}</p>
        </div>
      </div>
      <div className="flex flex-col items-center text-center gap-2">
        <div className="flex gap-5 p-1 mb-5">
          <img className="w-8 h-8 cursor-pointer" src={assets.shuffle_icon} alt="" />
          
          {/* Implement the previous button */}
          <img
            onClick={previous}
            className="w-8 h-8 cursor-pointer"
            src={assets.prev_icon}
            alt="Previous"
          />
          
          {/* Implement the play/pause toggle */}
          {playStatus ? (
            <img
              onClick={pause}
              className="w-8 h-8 cursor-pointer"
              src={assets.pause_icon}
              alt="Pause"
            />
          ) : (
            <img
              onClick={play}
              className="w-8 h-8 cursor-pointer"
              src={assets.play_icon}
              alt="Play"
            />
          )}

          {/* Implement the next button */}
          <img
            onClick={next}
            className="w-8 h-8 cursor-pointer"
            src={assets.next_icon}
            alt="Next"
          />

          <img className="w-8 h-8 cursor-pointer" src={assets.loop_icon} alt="" />
        </div>
        <div className="flex items-center gap-5 p-2">
          <p className="text-xs">{time.currentTime.minute}:{time.currentTime.second}</p>
          <div
            ref={seekBg}
            onClick={seekSong}
            className="w-[30vw] max-w-[500px] bg-gray-300 rounded-full cursor-pointer"
          >
            <hr
              ref={seekBar}
              className="h-1 bg-blue-800 rounded-full"
            />
          </div>
          <p className="text-xs">{time.totalTime.minute}:{time.totalTime.second}</p>
        </div>
      </div>
      <div className="flex flex-col items-center gap-2 opacity-75">
        <div className="flex gap-6 mb-9">
          <img className="w-6" src={assets.mic_icon} alt="" />
          <img className="w-6" src={assets.queue_icon} alt="" />
          <img className="w-6" src={assets.speaker_icon} alt="" />
          <img className="w-6" src={assets.volume_icon} alt="" />
          <img className="w-6" src={assets.zoom_icon} alt="" />
        </div>
      </div>
    </div>
  );
}

export default Player;
